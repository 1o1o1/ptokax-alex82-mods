...
// alex82 ... Перенесли сюда  обработку таймеров из ReceiveLoop, и тем самым повысили точность срабатывания в 2 раза.
#ifndef _WIN32
	timespec ts;
	#ifdef __MACH__
		mach_timespec_t mts;
	...
	if(clsServerManager::bServerTerminated == false) {
...
#if defined(_WIN32) && !defined(_WIN_IOT)
		// alex82 ... Настройка интервала между служебными циклами
        srvLoopTimer = SetTimer(NULL, 0, clsSettingManager::mPtr->i16Shorts[SETSHORT_SERVICE_LOOP_INTERVAL], NULL);
		...
		// alex82 ... Перенесено из HubCommands.cpp
		...
		if(CheckSprintf(iMsgLen, clsServerManager::szGlobalBufferSize, "clsServiceLoop::Looper::restart_message") == true) {
			clsGlobalDataQueue::mPtr->AddQueueItem(clsServerManager::pGlobalBuffer, iMsgLen, NULL, 0, clsGlobalDataQueue::CMD_CHAT);
		}
...
//            clsUdpDebug::mPtr->BroadcastFormat("[SYS] Range Banned ip %s - connection closed.", sIP);
	...
	// alex82 ... Отключение задержки перед отправкой $Lock
    if(clsSettingManager::mPtr->bBools[SETBOOL_LOCK_DELAYED] == false) {
    ...
    while(nextUser != 0 && clsServerManager::bServerTerminated == false) {
        ...
        switch(curUser->ui8State) {
            ...
            case User::STATE_ADDME: {
				...
				if(clsSettingManager::mPtr->bBools[SETBOOL_SHOW_WELCOME] == true) {
					...
					if(clsSettingManager::mPtr->sTexts[SETTXT_HUB_TOPIC] == NULL) {
						curUser->SendFormat("clsServiceLoop::ReceiveLoop->User::STATE_ADDME", true, "$HubName %s|", clsSettingManager::mPtr->sTexts[SETTXT_HUB_NAME]);
					} else {
						curUser->SendFormat("clsServiceLoop::ReceiveLoop->User::STATE_ADDME", true, "$HubName %s - %s|", clsSettingManager::mPtr->sTexts[SETTXT_HUB_NAME], clsSettingManager::mPtr->sTexts[SETTXT_HUB_TOPIC]);
					}
				}
            ...
            case User::STATE_ADDME_1LOOP: {
				...
				if(((curUser->ui32InfoBits & User::INFOBIT_HIDDEN) == User::INFOBIT_HIDDEN) == false) {
...
//                if(sqldb) sqldb->AddVisit(curUser);
				...
				if(((curUser->ui32InfoBits & User::INFOBIT_HIDDEN) == User::INFOBIT_HIDDEN) == false) {
	                ...
	                if(((curUser->ui32BoolBits & User::BIT_OPERATOR) == User::BIT_OPERATOR) == true && ((curUser->ui32InfoBits & User::INFOBIT_HIDE_KEY) == User::INFOBIT_HIDE_KEY) == false) {
    ...
    while(nextUser != 0 && clsServerManager::bServerTerminated == false) {
        ...
        switch(curUser->ui8State) {
            case User::STATE_ADDME_2LOOP: {
                ...
                if(((curUser->ui32SupportBits & User::SUPPORTBIT_USERIP2) == User::SUPPORTBIT_USERIP2) == true && clsProfileManager::mPtr->IsAllowed(curUser, clsProfileManager::SENDALLUSERIP) == false) {
