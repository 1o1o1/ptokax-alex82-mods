...
static int RegBot(lua_State * L) {
	...
	// alex82 ... Добавили альтернативную функцию для создания бота с полноценным $MyINFO
	bool bFullMyINFO = false;
	if(lua_gettop(L) == 3) bFullMyINFO = true;

	if(lua_gettop(L) != 4 && bFullMyINFO == false) {
        luaL_error(L, "bad argument count to 'RegBot' (3 or 4 expected, got %d)", lua_gettop(L));
        lua_settop(L, 0);
	...
	if (bFullMyINFO == false) {
	    ...
	    pNewBot = ScriptBot::CreateScriptBot(nick, szNickLen, description, szDescrLen, email, szEmailLen, bIsOP);
	} else {
		if(lua_type(L, 1) != LUA_TSTRING || lua_type(L, 2) != LUA_TSTRING || lua_type(L, 3) != LUA_TBOOLEAN) {
	        luaL_checktype(L, 1, LUA_TSTRING);
	        luaL_checktype(L, 2, LUA_TSTRING);
	    ...
	    char *nick = (char *)lua_tolstring(L, 1, &szNickLen);
	    char *myinfo = (char *)lua_tolstring(L, 2, &szMyINFOLen);

	    bool bIsOP = lua_toboolean(L, 3) == 0 ? false : true;

	    if(szNickLen == 0 || szNickLen > 64 || strpbrk(nick, " $|") != NULL ||
	        clsHashManager::mPtr->FindUser(nick, szNickLen) != NULL || clsReservedNicksManager::mPtr->CheckReserved(nick, HashNick(nick, szNickLen)) != false) {
			lua_settop(L, 0);
		...
		if (memcmp(test_myinfo, myinfo, szNickLen+14) != 0) {
	        luaL_error(L, "invalid MyINFO string");
	        lua_settop(L, 0);
	    ...
	    pNewBot = ScriptBot::CreateScriptBot(nick, szNickLen, myinfo, szMyINFOLen, bIsOP);
	}
...
static int BotMyINFO(lua_State * L) {
	if(lua_gettop(L) != 2) {
        luaL_error(L, "bad argument count to 'BotMyINFO' (2 expected, got %d)", lua_gettop(L));
        lua_settop(L, 0);
    ...
    size_t szNickLen, szMyINFOLen;
    char * botnick = (char *)lua_tolstring(L, 1, &szNickLen);
 	char *myinfo = (char *)lua_tolstring(L, 2, &szMyINFOLen);
	...
	char test_myinfo[(64+15)];
	sprintf(test_myinfo,"$MyINFO $ALL %s ",botnick);
	if (memcmp(test_myinfo, myinfo, szNickLen+14) != 0) {
		luaL_error(L, "invalid MyINFO string");
		lua_settop(L, 0);
    ...
    while(next != NULL) {
		...
		if(strcasecmp(botnick, cur->sNick) == 0) {
			bool bAddPipe = false;
			if (myinfo[szMyINFOLen-1] != '|') bAddPipe = true;
			
			clsUsers::mPtr->DelBotFromMyInfos(cur->sMyINFO);
			
			size_t szWantLen = szMyINFOLen;
			if (bAddPipe == true) szWantLen++;
			
			if (szWantLen != strlen(cur->sMyINFO)) {
				char * pOldBuf = cur->sMyINFO;
#ifdef _WIN32
				cur->sMyINFO = (char *)HeapReAlloc(clsServerManager::hPtokaXHeap, HEAP_NO_SERIALIZE, (void *)pOldBuf, szWantLen+1);
#else
				cur->sMyINFO = (char *)realloc(pOldBuf, szWantLen+1);
#endif
				if (cur->sMyINFO == NULL) {
					...
					AppendDebugLogFormat("[MEM] Cannot reallocate %" PRIu64 " bytes in Core.BotMyINFO\n", (uint64_t)szWantLen+1);
		    ...
		    memcpy(cur->sMyINFO, myinfo, szMyINFOLen);
			if (bAddPipe == true) cur->sMyINFO[szMyINFOLen] = '|';
		    cur->sMyINFO[szWantLen] = '\0';
			
			clsUsers::mPtr->AddBot2MyInfos(cur->sMyINFO);
			clsGlobalDataQueue::mPtr->AddQueueItem(cur->sMyINFO, strlen(cur->sMyINFO), NULL, 0, clsGlobalDataQueue::CMD_MYINFO);
...
static int GetUserData(lua_State * L) {
    ...
    switch(ui8DataId) {
		...
		// alex82 ... Are user supports zlib compression?
        case 64:
        	lua_pushliteral(L, "bZPipe");
			(u->ui32SupportBits & User::SUPPORTBIT_ZPIPE) == User::SUPPORTBIT_ZPIPE ? lua_pushboolean(L, 1) : lua_pushboolean(L, 0);
        	lua_rawset(L, 1);
            break;
		// alex82 ... Скрытие юзера
        case 65:
        	lua_pushliteral(L, "bHidden");
			(u->ui32InfoBits & User::INFOBIT_HIDDEN) == User::INFOBIT_HIDDEN ? lua_pushboolean(L, 1) : lua_pushboolean(L, 0);
        	lua_rawset(L, 1);
            break;
		// alex82 ... Подавляем $Quit для юзера
        case 66:
        	lua_pushliteral(L, "bNoQuit");
			(u->ui32InfoBits & User::INFOBIT_NO_QUIT) == User::INFOBIT_NO_QUIT ? lua_pushboolean(L, 1) : lua_pushboolean(L, 0);
        	lua_rawset(L, 1);
            break;
		// alex82 ... Прячем ключ юзера
        case 67:
        	lua_pushliteral(L, "bHiddenKey");
			(u->ui32InfoBits & User::INFOBIT_HIDE_KEY) == User::INFOBIT_HIDE_KEY ? lua_pushboolean(L, 1) : lua_pushboolean(L, 0);
        	lua_rawset(L, 1);
            break;
		// alex82 ... Изоляция юзера
        case 68:
        	lua_pushliteral(L, "bIsolated");
			(u->ui32InfoBits & User::INFOBIT_ISOLATED) == User::INFOBIT_ISOLATED ? lua_pushboolean(L, 1) : lua_pushboolean(L, 0);
        	lua_rawset(L, 1);
            break;
        default:
...
//------------------------------------------------------------------------------
// alex82 ... Изоляция юзера
static int SendToNonisolated(lua_State * L) {
	if(lua_gettop(L) != 1) {
        luaL_error(L, "bad argument count to 'SendToNonisolated' (1 expected, got %d)", lua_gettop(L));
        lua_settop(L, 0);
        return 0;
    }
    ...
    if(lua_gettop(L) == 1) {
        ...
        if(sData[0] != '\0' && szLen < 128001) {
			if(sData[szLen-1] != '|') {
                ...
                clsServerManager::pGlobalBuffer[szLen] = '|';
                clsServerManager::pGlobalBuffer[szLen+1] = '\0';
				clsGlobalDataQueue::mPtr->AddQueueItem(clsServerManager::pGlobalBuffer, szLen+1, NULL, 0, clsGlobalDataQueue::CMD_CHAT);
			} else {
				clsGlobalDataQueue::mPtr->AddQueueItem(sData, szLen, NULL, 0, clsGlobalDataQueue::CMD_CHAT);
            }
        }
    }
...
// alex82 ... Добавили функции статистики
static int GetDCStats(lua_State * L) {
	if(lua_gettop(L) != 0) {
        luaL_error(L, "bad argument count to 'GetDCStats' (0 expected, got %d)", lua_gettop(L));
        lua_settop(L, 0);
    ...
    lua_pushliteral(L, "iChatCount");
    lua_pushnumber(L, clsDcCommands::mPtr->iStatChat);
    lua_rawset(L, i);

    lua_pushliteral(L, "iUnknownCount");
    lua_pushnumber(L, clsDcCommands::mPtr->iStatCmdUnknown);
    lua_rawset(L, i);

    lua_pushliteral(L, "iToCount");
    lua_pushnumber(L, clsDcCommands::mPtr->iStatCmdTo);
    lua_rawset(L, i);

    lua_pushliteral(L, "iKeyCount");
    lua_pushnumber(L, clsDcCommands::mPtr->iStatCmdKey);
    lua_rawset(L, i);

    lua_pushliteral(L, "iSupportsCount");
    lua_pushnumber(L, clsDcCommands::mPtr->iStatCmdSupports);
    lua_rawset(L, i);

    lua_pushliteral(L, "iMyINFOCount");
    lua_pushnumber(L, clsDcCommands::mPtr->iStatCmdMyInfo);
    lua_rawset(L, i);

    lua_pushliteral(L, "iValidateNickCount");
    lua_pushnumber(L, clsDcCommands::mPtr->iStatCmdValidate);
    lua_rawset(L, i);

    lua_pushliteral(L, "iGetINFOCount");
    lua_pushnumber(L, clsDcCommands::mPtr->iStatCmdGetInfo);
    lua_rawset(L, i);

    lua_pushliteral(L, "iMyPassCount");
    lua_pushnumber(L, clsDcCommands::mPtr->iStatCmdMyPass);
    lua_rawset(L, i);

    lua_pushliteral(L, "iVersionCount");
    lua_pushnumber(L, clsDcCommands::mPtr->iStatCmdVersion);
    lua_rawset(L, i);

    lua_pushliteral(L, "iGetNickListCount");
    lua_pushnumber(L, clsDcCommands::mPtr->iStatCmdGetNickList);
    lua_rawset(L, i);

    lua_pushliteral(L, "iSearchCount");
    lua_pushnumber(L, clsDcCommands::mPtr->iStatCmdSearch);
    lua_rawset(L, i);

    lua_pushliteral(L, "iSRCount");
    lua_pushnumber(L, clsDcCommands::mPtr->iStatCmdSR);
    lua_rawset(L, i);

    lua_pushliteral(L, "iCTMCount");
    lua_pushnumber(L, clsDcCommands::mPtr->iStatCmdConnectToMe);
    lua_rawset(L, i);

    lua_pushliteral(L, "iRevCTMCount");
    lua_pushnumber(L, clsDcCommands::mPtr->iStatCmdRevCTM);
    lua_rawset(L, i);

    lua_pushliteral(L, "iBotINFOCount");
    lua_pushnumber(L, clsDcCommands::mPtr->iStatBotINFO);
    lua_rawset(L, i);

    lua_pushliteral(L, "iCloseCount");
    lua_pushnumber(L, clsDcCommands::mPtr->iStatCmdClose);
    lua_rawset(L, i);

	return 1;
}

static int GetNetStats(lua_State * L) {
	if(lua_gettop(L) != 0) {
        luaL_error(L, "bad argument count to 'GetNetStats' (0 expected, got %d)", lua_gettop(L));
        lua_settop(L, 0);
	...
	if(clsServiceLoop::mPtr == NULL) {
        luaL_error(L, "service loop are not started");
    ...
    lua_pushliteral(L, "iSendRests");
    lua_pushnumber(L, clsServiceLoop::mPtr->ui32LastSendRest);
    lua_rawset(L, i);

    lua_pushliteral(L, "iSendRestsPeak");
    lua_pushnumber(L, clsServiceLoop::mPtr->ui32SendRestsPeak);
    lua_rawset(L, i);

    lua_pushliteral(L, "iRecvRests");
    lua_pushnumber(L, clsServiceLoop::mPtr->ui32LastRecvRest);
    lua_rawset(L, i);

    lua_pushliteral(L, "iRecvRestsPeak");
    lua_pushnumber(L, clsServiceLoop::mPtr->ui32RecvRestsPeak);
    lua_rawset(L, i);

    lua_pushliteral(L, "iCompressionSaved");
    lua_pushnumber(L, (double)clsServerManager::ui64BytesSentSaved);
    lua_rawset(L, i);

    lua_pushliteral(L, "iCompressedCount");
    lua_pushnumber(L, clsDcCommands::mPtr->iStatZPipe);
    lua_rawset(L, i);

    lua_pushliteral(L, "iDataSent");
    lua_pushnumber(L, (double)clsServerManager::ui64BytesSent);
    lua_rawset(L, i);

    lua_pushliteral(L, "iDataRecv");
    lua_pushnumber(L, (double)clsServerManager::ui64BytesRead);
    lua_rawset(L, i);

    lua_pushliteral(L, "iTx");
    lua_pushnumber(L, clsServerManager::ui32ActualBytesSent);
    lua_rawset(L, i);

    lua_pushliteral(L, "iTx60sec");
    lua_pushnumber(L, clsServerManager::ui32AverageBytesSent/60);
    lua_rawset(L, i);

    lua_pushliteral(L, "iRx");
    lua_pushnumber(L, clsServerManager::ui32ActualBytesRead);
    lua_rawset(L, i);

    lua_pushliteral(L, "iRx60sec");
    lua_pushnumber(L, clsServerManager::ui32AverageBytesRead/60);
    lua_rawset(L, i);
...
static int HideUser(lua_State * L) {
	...
	bool bHide = lua_toboolean(L, 2) == 0 ? false : true;
    ...
    if(u != NULL) {
		if (bHide == true && ((u->ui32InfoBits & User::INFOBIT_HIDDEN) == User::INFOBIT_HIDDEN) == false) {
			char msg[72];

			u->ui32InfoBits |= User::INFOBIT_HIDDEN;

		    if(u->ui8State >= User::STATE_ADDME_2LOOP) {
				clsUsers::mPtr->DelFromNickList(u->sNick, ((u->ui32BoolBits & User::BIT_OPERATOR) == User::BIT_OPERATOR && ((u->ui32InfoBits & User::INFOBIT_HIDE_KEY) == User::INFOBIT_HIDE_KEY) == false));
				clsUsers::mPtr->DelFromUserIP(u);
		...
		else if (bHide == false && ((u->ui32InfoBits & User::INFOBIT_HIDDEN) == User::INFOBIT_HIDDEN) == true) {
		    if(u->ui8State > User::STATE_ADDME_2LOOP) {
		        clsServerManager::ui32Logged++;
		    }
			if(u->ui8State >= User::STATE_ADDME_2LOOP) {
				...
				if(((u->ui32BoolBits & User::BIT_HAVE_SHARECOUNTED) == User::BIT_HAVE_SHARECOUNTED) == false) {
					clsServerManager::ui64TotalShare += u->ui64SharedSize;
					u->ui32BoolBits |= User::BIT_HAVE_SHARECOUNTED;
				}

				clsGlobalDataQueue::mPtr->UserIPStore(u);

				switch(clsSettingManager::mPtr->ui8FullMyINFOOption) {
					case 0:
						clsGlobalDataQueue::mPtr->AddQueueItem(u->sMyInfoLong, u->ui16MyInfoLongLen, NULL, 0, clsGlobalDataQueue::CMD_MYINFO);
						break;
					case 1:
						clsGlobalDataQueue::mPtr->AddQueueItem(u->sMyInfoShort, u->ui16MyInfoShortLen, u->sMyInfoLong, u->ui16MyInfoLongLen, clsGlobalDataQueue::CMD_MYINFO);
						break;
					case 2:
						clsGlobalDataQueue::mPtr->AddQueueItem(u->sMyInfoShort, u->ui16MyInfoShortLen, NULL, 0, clsGlobalDataQueue::CMD_MYINFO);
						break;
					default:
						break;
				}
				// alex82 ... Прячем ключ юзера
				if(((u->ui32BoolBits & User::BIT_OPERATOR) == User::BIT_OPERATOR) == true && ((u->ui32InfoBits & User::INFOBIT_HIDE_KEY) == User::INFOBIT_HIDE_KEY) == false) {
					clsGlobalDataQueue::mPtr->OpListStore(u->sNick);
				}
			}
...
static int UserNoQuit(lua_State * L) {
	...
	bool bNoQuit = lua_toboolean(L, 2) == 0 ? false : true;
    ...
    if(u != NULL) {
		if (bNoQuit == true && ((u->ui32InfoBits & User::INFOBIT_NO_QUIT) == User::INFOBIT_NO_QUIT) == false) {
			u->ui32InfoBits |= User::INFOBIT_NO_QUIT;
		}
		else if (bNoQuit == false && ((u->ui32InfoBits & User::INFOBIT_NO_QUIT) == User::INFOBIT_NO_QUIT) == true) {
			u->ui32InfoBits &= ~User::INFOBIT_NO_QUIT;
		}
		lua_pushboolean(L, 1);
	} else {
...
static int HideUserKey(lua_State * L) {
    ...
    if(u != NULL) {
		char msg[80];
		if (bHide == true && ((u->ui32InfoBits & User::INFOBIT_HIDE_KEY) == User::INFOBIT_HIDE_KEY) == false) {

			u->ui32InfoBits |= User::INFOBIT_HIDE_KEY;

		    if(((u->ui32BoolBits & User::BIT_OPERATOR) == User::BIT_OPERATOR) == true && u->ui8State >= User::STATE_ADDME_2LOOP && ((u->ui32InfoBits & User::INFOBIT_HIDDEN) == User::INFOBIT_HIDDEN) == false) {
				clsUsers::mPtr->DelFromOpList(u->sNick);
		...
		else if (bHide == false && ((u->ui32InfoBits & User::INFOBIT_HIDE_KEY) == User::INFOBIT_HIDE_KEY) == true) {
			if(((u->ui32BoolBits & User::BIT_OPERATOR) == User::BIT_OPERATOR) == true && u->ui8State >= User::STATE_ADDME_2LOOP && ((u->ui32InfoBits & User::INFOBIT_HIDDEN) == User::INFOBIT_HIDDEN) == false) {
				clsUsers::mPtr->Add2OpList(u);
				clsGlobalDataQueue::mPtr->OpListStore(u->sNick);
			}
...
//------------------------------------------------------------------------------
// alex82 ... Изоляция юзера
static int IsolateUser(lua_State * L) {
	if(lua_gettop(L) != 2) {
		luaL_error(L, "bad argument count to 'IsolateUser' (2 expected, got %d)", lua_gettop(L));
		lua_settop(L, 0);
	...
	bool bIsolated = lua_toboolean(L, 2) == 0 ? false : true;
    
    User *u = ScriptGetUser(L, 2, "IsolateUser");

    lua_settop(L, 0);

    if(u != NULL) {
		if (bIsolated == true && ((u->ui32InfoBits & User::INFOBIT_ISOLATED) == User::INFOBIT_ISOLATED) == false) {
			u->ui32InfoBits |= User::INFOBIT_ISOLATED;
		}
		else if (bIsolated == false && ((u->ui32InfoBits & User::INFOBIT_ISOLATED) == User::INFOBIT_ISOLATED) == true) {
			u->ui32InfoBits &= ~User::INFOBIT_ISOLATED;
		}
		lua_pushboolean(L, 1);
	} else {
...
static const luaL_Reg core[] = {
    ...
    { "SendToAll", SendToAll }, 
	// alex82 ... Изоляция юзера
	{ "SendToNonisolated", SendToNonisolated }, 
