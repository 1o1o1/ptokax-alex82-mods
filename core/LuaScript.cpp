...
ScriptBot * ScriptBot::CreateScriptBot(char * sBotNick, const size_t &szNickLen, char * sBotMyINFO, const size_t &szMyINFOLen, const bool &bOP) {
...
#endif
    if(pScriptBot->sNick == NULL) {
		AppendDebugLogFormat("[MEM] Cannot allocate %" PRIu64 " bytes for sNick in ScriptBot::CreateScriptBot\n", (uint64_t)(szNickLen+1));
...
#endif
    if(pScriptBot->sMyINFO == NULL) {
		AppendDebugLogFormat("[MEM] Cannot allocate %" PRIu64 " bytes for sMyINFO in ScriptBot::CreateScriptBot\n", (uint64_t)(szMyINFOLen+1));

        delete pScriptBot;
		return NULL;
    }

    memcpy(pScriptBot->sMyINFO, sBotMyINFO, szMyINFOLen);
	if (bAddPipe == true) pScriptBot->sMyINFO[szMyINFOLen] = '|';
    pScriptBot->sMyINFO[szWantLen-1] = '\0';
...
static void AddSettingIds(lua_State * L) {
	...
	const uint8_t ui8Bools[] = { SETBOOL_ANTI_MOGLO, SETBOOL_AUTO_START, SETBOOL_REDIRECT_ALL, SETBOOL_REDIRECT_WHEN_HUB_FULL, SETBOOL_AUTO_REG, SETBOOL_REG_ONLY, 
		...
		SETBOOL_LOG_SCRIPT_ERRORS, CRAP_SETBOOL_NO_QUACK_SUPPORTS,
			// alex82 ... Добавленные настройки
		SETBOOL_SHOW_WELCOME, SETBOOL_BLOCK_UNKNOWN_CMD, SETBOOL_CHECK_KEYS, SETBOOL_USE_COMPRESSION,
		SETBOOL_SCRIPT_ERRORS_TO_OPS, SETBOOL_SCRIPT_STACK_TRACEBACK, SETBOOL_KEEP_MAGIC_BYTE, SETBOOL_LOCK_DELAYED,
	...
	const char * pBoolsNames[] = { "AntiMoGlo", "AutoStart", "RedirectAll", "RedirectWhenHubFull", "AutoReg", "RegOnly",
		...
		"LogScriptErrors", "DisallowBadSupports",
		// alex82 ... Добавленные настройки
		"ShowWelcome", "BlockUnknownCmd", "CheckKeys", "UseCompression",
		"ScriptErrorsToOps", "ScriptStackTraceback", "KeepMagicByte", "SendLockDelayed",
	...
	const uint8_t ui8Numbers[] = { SETSHORT_MAX_USERS, SETSHORT_MIN_SHARE_LIMIT, SETSHORT_MIN_SHARE_UNITS, SETSHORT_MAX_SHARE_LIMIT, SETSHORT_MAX_SHARE_UNITS, 
		...
		SETSHORT_SEARCH_INTERVAL_MESSAGES, SETSHORT_SEARCH_INTERVAL_TIME, SETSHORT_MAX_CONN_SAME_IP, SETSHORT_MIN_RECONN_TIME,
			// alex82 ... Добавленные настройки
		SETSHORT_MAX_TEMP_BAN_TIME, SETSHORT_SERVICE_LOOP_INTERVAL,
	...
	const char * pNumbersNames[] = { "MaxUsers", "MinShareLimit", "MinShareUnits", "MaxShareLimit", "MaxShareUnits",
		...
		// alex82 ... Добавленные настройки
		"MaxTempBanTime", "ServiceLoopInterval", 
	...
	const uint8_t ui8Strings[] = { SETTXT_HUB_NAME, SETTXT_ADMIN_NICK, SETTXT_HUB_ADDRESS, SETTXT_TCP_PORTS, SETTXT_UDP_PORT, SETTXT_HUB_DESCRIPTION, SETTXT_REDIRECT_ADDRESS, 
		...
		// alex82 ... Добавленные настройки
		SETTXT_LOCALE_WIN, SETTXT_LOCALE_NIX,

		SETTXT_ENCODING, SETTXT_DB_HOST, SETTXT_DB_PORT, SETTXT_DB_DBNAME, SETTXT_DB_USER, SETTXT_DB_PASS,
	};
	...
	const char * pStringsNames[] = { "HubName", "AdminNick", "HubAddress", "TCPPorts", "UDPPort", "HubDescription", "MainRedirectAddress",
		...
		"Encoding", "DBHost", "DBPort", "DBName", "DBUser", "DBPass",
	};
...
static int os_clock (lua_State *L) {
	...
	lua_pushnumber(L, (lua_Number)(tv.tv_sec-clsServerManager::tStartTime+(float)((uint16_t)(tv.tv_usec/1000))/1000));
	return 1;
}
#endif
#endif
...
bool ScriptStart(Script * cur) {
	...
	if(lua_istable(cur->pLUA, -1)) {
		lua_pushcfunction(cur->pLUA, os_clock);
		lua_setfield(cur->pLUA, -2, "clock");
	...
	// alex82 ... Добавили функцию print
	lua_pushcfunction(cur->pLUA, LuaPrint);
	lua_setglobal(cur->pLUA, "print");
	...
	if(luaL_dofile(cur->pLUA, (clsServerManager::sScriptPath+cur->sName).c_str()) == 0) {
		...
		// alex82 ... Отправляем сообщение В ОпЧат
		if(clsSettingManager::mPtr->bBools[SETBOOL_SCRIPT_ERRORS_TO_OPS] == true && clsSettingManager::mPtr->bBools[SETBOOL_REG_OP_CHAT] == true) {
			int iLen = sprintf(clsServerManager::pGlobalBuffer, "%s $<%s> %s|", clsSettingManager::mPtr->sTexts[SETTXT_OP_CHAT_NICK], clsSettingManager::mPtr->sTexts[SETTXT_OP_CHAT_NICK], sMsg.c_str());
...
void ScriptOnStartup(Script * cur) {
	// alex82 ... Отключение stack traceback
	int iTraceback = 0;
	if(clsSettingManager::mPtr->bBools[SETBOOL_SCRIPT_STACK_TRACEBACK] == true) {
...
#else
		// alex82 ... Здесь и далее слегка переписано
		if((ui64ActualMillis - pCurTmr->ui64LastTick) >= pCurTmr->ui64Interval) {
			...
			if(clsSettingManager::mPtr->bBools[SETBOOL_SCRIPT_STACK_TRACEBACK] == true) {
	            ...
	            iTraceback = lua_gettop(pCurTmr->pLua);
			}
