        ...
        if(pQueueItems != NULL) {
            ...
            while(pNext != NULL) {
                ...
                switch(pCur->ui8CommandType) {
                    ...
                    case CMD_CHAT:
                        if(pCur->pCommand1 != NULL && ((pUser->ui32InfoBits & User::INFOBIT_ISOLATED) == User::INFOBIT_ISOLATED) == false) {
                            AddDataToQueue(GlobalQueues[ui32QueueType], pCur->pCommand1, pCur->szLen1);
                        }
                        break;
                    case CMD_HUBNAME:
    ...
    if(clsServerManager::ui8SrCntr == 0) {
		...
		if(cmdActiveSearch != NULL) {
			if(pUser->ui64SharedSize == 0) {
				...
				if(clsSettingManager::mPtr->bBools[SETBOOL_USE_COMPRESSION] == true && ((pUser->ui32SupportBits & User::SUPPORTBIT_ZPIPE) == User::SUPPORTBIT_ZPIPE) == true) {
