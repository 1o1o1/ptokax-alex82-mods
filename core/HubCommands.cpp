...
#endif
//---------------------------------------------------------------------------
// alex82 ... Unknown command blocking
#define COMMAND_NOT_FOUND if(clsSettingManager::mPtr->bBools[SETBOOL_BLOCK_UNKNOWN_CMD] == true) {SendNoPermission(pUser, bFromPM); return true;} else return false;
//---------------------------------------------------------------------------
    ...
    switch(tolower(sCommand[0])) {
...
#elif _WITH_MYSQL
					if(DBMySQL::mPtr->SearchNick(sCommand, (uint8_t)dlen-8, pUser, bFromPM) == true) {
						UncountDeflood(pUser, bFromPM);
						return true;
					}
#endif
        ...
        case 'n':
			...
			if(strncasecmp(sCommand+1, "icktempban ", 11) == 0) {
				...
				// alex82 ... Максимальная продолжительность временного бана
				if((uint32_t)((ban_time-acc_time)/86400) > (uint32_t)clsSettingManager::mPtr->i16Shorts[SETSHORT_MAX_TEMP_BAN_TIME]) {
                    pUser->SendFormatCheckPM("clsHubCommands::DoCommand->nicktempban::check_time", bFromPM == true ? clsSettingManager::mPtr->sPreTexts[clsSettingManager::SETPRETXT_HUB_SEC] : NULL, true, "<%s> %s %s!|", clsSettingManager::mPtr->sPreTexts[clsSettingManager::SETPRETXT_HUB_SEC], 
						clsLanguageManager::mPtr->sTexts[LAN_TEMP_BAN_TIME_CANNOT_EXCEED], formatTime(clsSettingManager::mPtr->i16Shorts[SETSHORT_MAX_TEMP_BAN_TIME]*1440));
					return true;
				}
        ...
        case 'r':
            ...
            // alex82 ... Hub-Commands: !reloadcountry
			if(dlen == 13 && strncasecmp(sCommand+1, "eloadcountry", 12) == 0) {
           	    if(pUser->i32Profile != 0) {
                    SendNoPermission(pUser, bFromPM);
                    return true;
                }
                ...
                if(clsSettingManager::mPtr->bBools[SETBOOL_SEND_STATUS_MESSAGES] == true) {
               	    clsGlobalDataQueue::mPtr->StatusMessageFormat("clsHubCommands::DoCommand->reloadcountry", "<%s> *** %s %s.|", clsSettingManager::mPtr->sPreTexts[clsSettingManager::SETPRETXT_HUB_SEC], pUser->sNick, clsLanguageManager::mPtr->sTexts[LAN_RELOAD_IPTOCOUNTRY_LWR]);
                }

                if(clsSettingManager::mPtr->bBools[SETBOOL_SEND_STATUS_MESSAGES] == false || ((pUser->ui32BoolBits & User::BIT_OPERATOR) == User::BIT_OPERATOR) == false) {
                    pUser->SendFormatCheckPM("clsHubCommands::DoCommand->reloadcountry2", bFromPM == true ? clsSettingManager::mPtr->sPreTexts[clsSettingManager::SETPRETXT_HUB_SEC] : NULL, true, "<%s> %s.|", clsSettingManager::mPtr->sPreTexts[clsSettingManager::SETPRETXT_HUB_SEC], clsLanguageManager::mPtr->sTexts[LAN_IPTOCOUNTRY_RELOADED]);
                }
                return true;
            }
        ...
        case 'o':
			...
			if(strncasecmp(sCommand+1, "p ", 2) == 0) {
				...
				if(((pOtherUser->ui32InfoBits & User::INFOBIT_HIDE_KEY) == User::INFOBIT_HIDE_KEY) == false) {
...
*/
		...
		case 'p':
			...
			if(strncasecmp(sCommand+1, "asswd ", 6) == 0) {
				...
				// alex82 ... Хеширование паролей
#ifdef HASH_PASS
				SHA256_Base64((unsigned char *)sCommand+7, szPassLen, (unsigned char *)pReg->sPass);
#else
    ...
    switch(clsBanManager::mPtr->BanIp(NULL, sCommand, sReason, pUser->sNick, bFull)) {
        case 0: {
            ...
            while(pNext != NULL) {
                ...
                // PPK don't nickban user with higher profile
				// alex82 ... Запретили юзерам с одинаковыми профилями банить друг друга
                if(pCur->i32Profile != -1 && pUser->i32Profile != 0 && pUser->i32Profile >= pCur->i32Profile) {
...
bool clsHubCommands::NickBan(User * pUser, char * sNick, char * sReason, bool bFromPM) {
	...
	// alex82 ... Запретили юзерам с одинаковыми профилями банить друг друга
    if(pReg != NULL && pUser->i32Profile != 0 && pUser->i32Profile >= pReg->ui16Profile) {
...
bool clsHubCommands::TempBan(User * pUser, char * sCommand, const size_t &dlen, bool bFromPM, bool bFull) {
    ...
    if(pOtherUser != NULL) {
		...
		// alex82 ... Максимальная продолжительность временного бана
		if((uint32_t)((ban_time-acc_time)/86400) > (uint32_t)clsSettingManager::mPtr->i16Shorts[SETSHORT_MAX_TEMP_BAN_TIME]) {
            pUser->SendFormatCheckPM("clsHubCommands::TempBan::check_time", bFromPM == true ? clsSettingManager::mPtr->sPreTexts[clsSettingManager::SETPRETXT_HUB_SEC] : NULL, true, "<%s> %s %s!|", clsSettingManager::mPtr->sPreTexts[clsSettingManager::SETPRETXT_HUB_SEC], 
				clsLanguageManager::mPtr->sTexts[LAN_TEMP_BAN_TIME_CANNOT_EXCEED], formatTime(clsSettingManager::mPtr->i16Shorts[SETSHORT_MAX_TEMP_BAN_TIME]*1440));
			return true;
		}
...
bool clsHubCommands::TempBanIp(User * pUser, char * sCommand, const size_t &dlen, bool bFromPM, bool bFull) {
	...
	// alex82 ... Максимальная продолжительность временного бана
	if((uint32_t)((ban_time-acc_time)/86400) > (uint32_t)clsSettingManager::mPtr->i16Shorts[SETSHORT_MAX_TEMP_BAN_TIME]) {
        pUser->SendFormatCheckPM("clsHubCommands::TempBanIp::check_time", bFromPM == true ? clsSettingManager::mPtr->sPreTexts[clsSettingManager::SETPRETXT_HUB_SEC] : NULL, true, "<%s> %s %s!|", clsSettingManager::mPtr->sPreTexts[clsSettingManager::SETPRETXT_HUB_SEC], 
			clsLanguageManager::mPtr->sTexts[LAN_TEMP_BAN_TIME_CANNOT_EXCEED], formatTime(clsSettingManager::mPtr->i16Shorts[SETSHORT_MAX_TEMP_BAN_TIME]*1440));
		return true;
	}
...
bool clsHubCommands::TempNickBan(User * pUser, char * sNick, char * sTime, const size_t &szTimeLen, char * sReason, bool bFromPM, bool bNotNickBan/* = false*/) {
	...
	// alex82 ... Максимальная продолжительность временного бана
	if((uint32_t)((ban_time-acc_time)/86400) > (uint32_t)clsSettingManager::mPtr->i16Shorts[SETSHORT_MAX_TEMP_BAN_TIME]) {
		pUser->SendFormatCheckPM("clsHubCommands::TempNickBan::check_time", bFromPM == true ? clsSettingManager::mPtr->sPreTexts[clsSettingManager::SETPRETXT_HUB_SEC] : NULL, true, "<%s> %s %s!|", clsSettingManager::mPtr->sPreTexts[clsSettingManager::SETPRETXT_HUB_SEC],
			clsLanguageManager::mPtr->sTexts[LAN_TEMP_BAN_TIME_CANNOT_EXCEED], formatTime(clsSettingManager::mPtr->i16Shorts[SETSHORT_MAX_TEMP_BAN_TIME]*1440));
		return true;
	}
...
bool clsHubCommands::RangeTempBan(User * pUser, char * sCommand, const size_t &dlen, bool bFromPM, bool bFull) {
	...
	// alex82 ... Максимальная продолжительность временного бана
	if((uint32_t)((ban_time-acc_time)/86400) > (uint32_t)clsSettingManager::mPtr->i16Shorts[SETSHORT_MAX_TEMP_BAN_TIME]) {
        pUser->SendFormatCheckPM("clsHubCommands::RangeTempBan::check_time", bFromPM == true ? clsSettingManager::mPtr->sPreTexts[clsSettingManager::SETPRETXT_HUB_SEC] : NULL, true, "<%s> %s %s!|", clsSettingManager::mPtr->sPreTexts[clsSettingManager::SETPRETXT_HUB_SEC], 
			clsLanguageManager::mPtr->sTexts[LAN_TEMP_BAN_TIME_CANNOT_EXCEED], formatTime(clsSettingManager::mPtr->i16Shorts[SETSHORT_MAX_TEMP_BAN_TIME]*1440));
		return true;
	}
