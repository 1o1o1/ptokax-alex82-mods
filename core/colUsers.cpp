...
void clsUsers::Add2NickList(User * pUser) {
    ...
    if(((pUser->ui32BoolBits & User::BIT_OPERATOR) == User::BIT_OPERATOR) == false || ((pUser->ui32InfoBits & User::INFOBIT_HIDE_KEY) == User::INFOBIT_HIDE_KEY) == true) {
        return;
    }
...
void clsUsers::SendChat2All(User * pUser, char * sData, const size_t &szChatLen, void * pQueueItem) {
	...
	if (((pUser->ui32InfoBits & User::INFOBIT_ISOLATED) == User::INFOBIT_ISOLATED) == false) {
	    if(pQueueItem == NULL) {
		...
		pUser->SendCharDelayed(sData, szChatLen);
	}
}
//---------------------------------------------------------------------------
