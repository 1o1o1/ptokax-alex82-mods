...
void clsSettingManager::CreateDefaultMOTD() {
	#define DEFAULT_MOTD "Добро пожаловать на хаб, работающий под управлением PtokaX Mod"
#ifdef _WIN32
    sMOTD = (char *)HeapAlloc(clsServerManager::hPtokaXHeap, HEAP_NO_SERIALIZE, sizeof(DEFAULT_MOTD)+1);
#else
	sMOTD = (char *)malloc(sizeof(DEFAULT_MOTD)+1);
#endif
    if(sMOTD == NULL) {
		AppendDebugLog("%s - [MEM] Cannot allocate memory for sMOTD in clsSettingManager::CreateDefaultMOTD\n");
        exit(EXIT_FAILURE);
    }
    memcpy(sMOTD, DEFAULT_MOTD, sizeof(DEFAULT_MOTD));
    sMOTD[sizeof(DEFAULT_MOTD)] = '\0';
    ui16MOTDLen = sizeof(DEFAULT_MOTD);
}
//---------------------------------------------------------------------------
...
void clsSettingManager::Save() {
    ...
    for(size_t szi = 0; szi < SETBOOL_IDS_END; szi++) {
        ...
        if(SetBoolStr[szi][0] == '\0' || SetShortStr[szi][0] == '\0') {
            continue;
        }

    	// Don't save empty hint
    	if(SetBoolCom[szi] != NULL) {
			// alex82 ... Переносы строк
			fputs("\n# ", fSettingsFile);
			fputs(SetBoolCom[szi], fSettingsFile);
			fputs("\n", fSettingsFile);
		}

		// alex82 ... Сохраняем все настройки
    ...
    for(size_t szi = 0; szi < SETSHORT_IDS_END; szi++) {
        // Don't save setting with empty id
        if(SetShortStr[szi][0] == '\0' || SetShortStr[szi][0] == '\0') {
            continue;
        }
    ...
    for(size_t szi = 0; szi < SETTXT_IDS_END; szi++) {
        // Don't save setting with empty id
        if(SetTxtStr[szi][0] == '\0' || SetShortStr[szi][0] == '\0') {
            continue;
        }
...
void clsSettingManager::SetBool(const size_t &szBoolId, const bool &bValue) {
    ...
    switch(szBoolId) {
		...
		case SETBOOL_SHOW_WELCOME:
			if (bValue == true) UpdateHubNameWelcome();
...
void clsSettingManager::UpdateHubNameWelcome() {
    if(bUpdateLocked == true || clsSettingManager::mPtr->bBools[SETBOOL_SHOW_WELCOME] == false) {
        return;
    }
...
maxusers:
	...
	const uint8_t ui8Strings[] = { SETTXT_HUB_NAME, SETTXT_HUB_ADDRESS,
#if defined(_WITH_POSTGRES) || defined(_WITH_MYSQL)
