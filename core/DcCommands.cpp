                            ...
                            if((pUser->ui32BoolBits & User::BIT_WAITING_FOR_PASS) == User::BIT_WAITING_FOR_PASS) {
                                ...
                                if(pUser->pLogInOut != NULL && pUser->pLogInOut->pBuffer != NULL) {
                                    ...
                                    if(((pUser->ui32BoolBits & User::BIT_OPERATOR) == User::BIT_OPERATOR) == false) {
										...
										if(((pUser->ui32InfoBits & User::INFOBIT_HIDE_KEY) == User::INFOBIT_HIDE_KEY) == false) {
    ...
    if(((pUser->ui32SupportBits & User::SUPPORTBIT_QUICKLIST) == User::SUPPORTBIT_QUICKLIST) == true &&
		...
		if(((pUser->ui32BoolBits & User::BIT_HAVE_GETNICKLIST) == User::BIT_HAVE_GETNICKLIST) == false) {
            ...
            if(((pUser->ui32SupportBits & User::SUPPORTBIT_NOHELLO) == User::SUPPORTBIT_NOHELLO) == false && clsUsers::mPtr->ui32NickListLen > 11) {
                // alex82 ... Отключение сжатия
				if(clsSettingManager::mPtr->bBools[SETBOOL_USE_COMPRESSION] == false || ((pUser->ui32SupportBits & User::SUPPORTBIT_ZPIPE) == User::SUPPORTBIT_ZPIPE) == false) {
...
void clsDcCommands::Key(User * pUser, char * sData, const uint32_t &ui32Len) {
	...
	// alex82 ... Отключение проверки ключа
    if(clsSettingManager::mPtr->bBools[SETBOOL_CHECK_KEYS] == true && (ui32Len < 6 || strcmp(Lock2Key(pUser->pLogInOut->pBuffer), sData+5) != 0)) {
			...
			if(sBanTime[5] == '\0' || sBanTime[5] == ' ') { // permban
				...
				if(cTime != '\0' && iTime > 0 && GenerateTempBanTime(cTime, iTime, acc_time, ban_time) == true) {
					...
					if((uint32_t)((ban_time-acc_time)/86400) > (uint32_t)clsSettingManager::mPtr->i16Shorts[SETSHORT_MAX_TEMP_BAN_TIME]) {
						pUser->SendFormat("clsDcCommands::Kick::check_time", true, "<%s> %s %s!|", clsSettingManager::mPtr->sPreTexts[clsSettingManager::SETPRETXT_HUB_SEC],
...
void clsDcCommands::MyPass(User * pUser, char * sData, const uint32_t &ui32Len) {
	...
	// alex82 ... Выпилили родное хеширование паролей, и впилили SHA256
    // if password is wrong, close the connection
#ifdef HASH_PASS
	char buff[SHA256_DIGEST_BASE64_LENGTH];
	SHA256_Base64((unsigned char *)sData+8, ui32Len-9, (unsigned char *)buff);
    if(strcmp(pReg->sPass, buff) != 0) {
#else
            ...
            case 'B': {
                if(((pUser->ui32BoolBits & User::BIT_PINGER) == User::BIT_PINGER) == false && szDataLen == 7 && memcmp(sSupport+1, "otINFO", 6) == 0) {
                    if(clsSettingManager::mPtr->bBools[SETBOOL_DONT_ALLOW_PINGERS] == true) {
						...
						// alex82 ... Включение/отключение приветствия
						if(clsSettingManager::mPtr->bBools[SETBOOL_SHOW_WELCOME] == true) {
							...
							if(clsSettingManager::mPtr->sTexts[SETTXT_HUB_TOPIC] == NULL) {
								pUser->SendFormat("clsDcCommands::Supports4", true, "$HubName %s|", clsSettingManager::mPtr->sTexts[SETTXT_HUB_NAME]);
							} else {
								pUser->SendFormat("clsDcCommands::Supports4", true, "$HubName %s - %s|", clsSettingManager::mPtr->sTexts[SETTXT_HUB_NAME], clsSettingManager::mPtr->sTexts[SETTXT_HUB_TOPIC]);
							}
						}
                    }
                }
...
void clsDcCommands::ProcessCmds(User * pUser) {
    ...
    while(next != NULL) {
        ...
        switch(cur->ui8Type) {
            case PrcsdUsrCmd::SUPPORTS: {
                ...
                if((pUser->ui32SupportBits & User::SUPPORTBIT_USERIP2) == User::SUPPORTBIT_USERIP2) {
    ...
    if((pUser->ui32BoolBits & User::BIT_PRCSD_MYINFO) == User::BIT_PRCSD_MYINFO) {
		...
		if(((pUser->ui32InfoBits & User::INFOBIT_HIDDEN) == User::INFOBIT_HIDDEN) == false) {
