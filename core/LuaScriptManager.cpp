...
void clsScriptManager::CheckForNewScripts() {
    ...
    if(hFile != -1) {
        do {
			...
			// alex82 ... locale.lua autostart
			AddScript(luafile.name, (stricmp(luafile.name, "locale.lua") == 0 ? true : false), false);
    ...
    while((p_dirent = readdir(p_scriptdir)) != NULL) {
		...
		// alex82 ... locale.lua autostart
		AddScript(p_dirent->d_name, (strcasecmp(p_dirent->d_name, "locale.lua") == 0 ? true : false), false);
    }
