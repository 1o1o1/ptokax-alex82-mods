...
DBMySQL::DBMySQL() {
	bConnected = false;
	pDBHandle = NULL;
	iLastReconnectAttempt = 0;

	if(clsSettingManager::mPtr->bBools[SETBOOL_ENABLE_DATABASE] == false || clsSettingManager::mPtr->sTexts[SETTXT_DB_PASS] == NULL) {
		return;
	}
	...
	// alex82 ... ѕри создании бд указываем нормальную кодировку, а не ту хрень, что была тут изначально.
	sprintf(sSQLCommand, 
		"CREATE TABLE IF NOT EXISTS userinfo ("
			...
			"email VARCHAR(96)"
		")"
		"DEFAULT CHARACTER SET %s COLLATE %s_general_ci",
		clsSettingManager::mPtr->sTexts[SETTXT_ENCODING], clsSettingManager::mPtr->sTexts[SETTXT_ENCODING]
	);
...
bool DBMySQL::Connect() {
	if (clsServerManager::ui64ActualTick != 0 && clsServerManager::ui64ActualTick - iLastReconnectAttempt <= 60 * 5) {
		return false;
	}
	
	iLastReconnectAttempt = clsServerManager::ui64ActualTick;
	
	if (pDBHandle != NULL) {
		mysql_close(pDBHandle);
		pDBHandle = NULL;
		
		AppendLog("DBMySQL: reconnecting to server...");
	}
	...
	int iTimeout = 3;
	mysql_options(pDBHandle, MYSQL_OPT_RECONNECT, &bReconnect);
	mysql_options(pDBHandle, MYSQL_OPT_CONNECT_TIMEOUT, &iTimeout);

	if(mysql_real_connect(pDBHandle, clsSettingManager::mPtr->sTexts[SETTXT_DB_HOST], clsSettingManager::mPtr->sTexts[SETTXT_DB_USER], clsSettingManager::mPtr->sTexts[SETTXT_DB_PASS], clsSettingManager::mPtr->sTexts[SETTXT_DB_DBNAME], atoi(clsSettingManager::mPtr->sTexts[SETTXT_DB_PORT]), NULL, 0) == NULL) {
		bConnected = false;
	...
	// alex82 ... ”станавливаем кодировку.
	sprintf(sSQLCommand, "SET NAMES %s", clsSettingManager::mPtr->sTexts[SETTXT_ENCODING]);

	if(mysql_query(pDBHandle, sSQLCommand) != 0) {
		clsUdpDebug::mPtr->BroadcastFormat("[LOG] DBMySQL set names failed: %s", mysql_error(pDBHandle));
	}
...
void DBMySQL::UpdateRecord(User * pUser) {
	...
	if(bConnected == false || mysql_ping(pDBHandle) != 0) {
		if (Connect()) {
			bConnected = true;
		} else {
			bConnected = false;
	...
	// alex82 ... ¬ыпилили преобразование в говнокодировку
	char sNick[129];
	if(mysql_real_escape_string(pDBHandle, sNick, pUser->sNick, pUser->ui8NickLen) == 0) {
		return;
	}
	...
	if(pUser->sDescription != NULL) {
		mysql_real_escape_string(pDBHandle, sDescription, pUser->sDescription, pUser->ui8DescriptionLen);
	} else {
		sDescription[0] = '\0';
	}
	...
	char sTag[385];
	if(pUser->sTag != NULL) {
		mysql_real_escape_string(pDBHandle, sTag, pUser->sTag, pUser->ui8TagLen);
	} else {
		sTag[0] = '\0';
	}
	...
	if(pUser->sConnection != NULL) {
		mysql_real_escape_string(pDBHandle, sConnection, pUser->sConnection, pUser->ui8ConnectionLen);
	} else {
		sConnection[0] = '\0';
	}
	...
	if(pUser->sEmail != NULL) {
		mysql_real_escape_string(pDBHandle, sEmail, pUser->sEmail, pUser->ui8EmailLen);
	} else {
		sEmail[0] = '\0';
	}
...
bool DBMySQL::SearchNick(char * sNick, const uint8_t &ui8NickLen, User * pUser, const bool &bFromPM) {
	...
	if(mysql_real_escape_string(pDBHandle, sEscapedNick, sNick, ui8NickLen) == 0) {
		return false;
	}
