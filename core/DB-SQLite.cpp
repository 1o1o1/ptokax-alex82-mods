...
void DBSQLite::UpdateRecord(User * pUser) {
	...
	char sNick[65];
	memcpy(sNick, pUser->sNick, pUser->ui8NickLen);
	sNick[pUser->ui8NickLen] = '\0';
	...
	if(pUser->sDescription != NULL) {
		memcpy(sDescription, pUser->sDescription, pUser->ui8DescriptionLen);
		sDescription[pUser->ui8DescriptionLen] = '\0';
	} else {
		sDescription[0] = '\0';
	}
	...
	char sTag[193];
	if(pUser->sTag != NULL) {
		memcpy(sTag, pUser->sTag, pUser->ui8TagLen);
		sTag[pUser->ui8TagLen] = '\0';
	} else {
		sTag[0] = '\0';
	}
	...
	if(pUser->sConnection != NULL) {
		memcpy(sConnection, pUser->sConnection, pUser->ui8ConnectionLen);
		sConnection[pUser->ui8ConnectionLen] = '\0';
	} else {
		sConnection[0] = '\0';
	}
	...
	if(pUser->sEmail != NULL) {
		memcpy(sEmail, pUser->sEmail, pUser->ui8EmailLen);
		sEmail[pUser->ui8EmailLen] = '\0';
	} else {
		sEmail[0] = '\0';
	}
