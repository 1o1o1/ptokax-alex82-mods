...
static void PushReg(lua_State * L, RegUser * r) {
	...
	lua_pushliteral(L, "iRegDate");
#if LUA_VERSION_NUM < 503
	lua_pushnumber(L, (lua_Number)r->tRegDate);
#else
	lua_pushinteger(L, (lua_Number)r->tRegDate);
#endif
	lua_rawset(L, i);

	lua_pushliteral(L, "iLastEnter");
#if LUA_VERSION_NUM < 503
	lua_pushnumber(L, (lua_Number)r->tLastEnter);
#else
	lua_pushinteger(L, (lua_Number)r->tLastEnter);
#endif
	lua_rawset(L, i);

	lua_pushliteral(L, "iOnlineTime");
#if LUA_VERSION_NUM < 503
	lua_pushnumber(L, (lua_Number)r->tOnlineTime);
#else
	lua_pushinteger(L, (lua_Number)r->tOnlineTime);
#endif
	lua_rawset(L, i);

	if (r->sCustom != NULL) {
	    lua_pushliteral(L, "sCustom");
	    lua_pushstring(L, r->sCustom);
	    lua_rawset(L, i);
	}
}
//------------------------------------------------------------------------------
...
static int SetCustom(lua_State * L) {
	if(lua_gettop(L) != 2) {
        luaL_error(L, "bad argument count to 'SetCustom' (2 expected, got %d)", lua_gettop(L));
        lua_settop(L, 0);
        lua_pushnil(L);
        return 1;
    }
    ...
    size_t szCustomLen;
    char * sCustom = (char *)lua_tolstring(L, 2, &szCustomLen);
    ...
    clsRegManager::mPtr->SetCustom(reg, sCustom, szCustomLen);
...
static int UpdateTimes(lua_State * L) {
	if(lua_gettop(L) != 0) {
        luaL_error(L, "bad argument count to 'UpdateTimes' (0 expected, got %d)", lua_gettop(L));
        lua_settop(L, 0);
        return 0;
    }

	clsRegManager::mPtr->UpdateTimes();

    return 0;
}
//------------------------------------------------------------------------------

static int SetTimes(lua_State * L) {
	if(lua_gettop(L) != 4) {
		luaL_error(L, "bad argument count to 'SetTimes' (4 expected, got %d)", lua_gettop(L));
		lua_settop(L, 0);
		lua_pushnil(L);
		return 1;
	}

	if(lua_type(L, 1) != LUA_TSTRING || lua_type(L, 2) != LUA_TNUMBER || lua_type(L, 3) != LUA_TNUMBER || lua_type(L, 4) != LUA_TNUMBER) {
		luaL_checktype(L, 1, LUA_TSTRING);
		luaL_checktype(L, 2, LUA_TNUMBER);
		luaL_checktype(L, 3, LUA_TNUMBER);
...
#if LUA_VERSION_NUM < 503
	reg->tRegDate = (time_t)lua_tonumber(L, 2);
	reg->tLastEnter = (time_t)lua_tonumber(L, 3);
	reg->tOnlineTime = (time_t)lua_tonumber(L, 4);
#else
	reg->tRegDate = (time_t)lua_tounsigned(L, 2);
	reg->tLastEnter = (time_t)lua_tounsigned(L, 3);
	reg->tOnlineTime = (time_t)lua_tounsigned(L, 4);
#endif
