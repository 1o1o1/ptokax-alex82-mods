...
void clsBanManager::Load() {
	...
	// alex82 ... Error processing
		...
		if (doc.Error() && doc.ErrorId() != TiXmlBase::TIXML_ERROR_OPENING_FILE){
			char msg[1024];
			int imsgLen = sprintf(msg, "Syntax error in cfg/BanList.xml: %s Line: %d", doc.ErrorDesc(), doc.ErrorRow());
			CheckSprintf(imsgLen, 1024, "clsBanManager::Load");
...
void clsBanManager::Save(bool bForce/* = false*/) {
    ...
    if(pRangeBanListS != NULL) {
        RangeBanItem *next = pRangeBanListS;
